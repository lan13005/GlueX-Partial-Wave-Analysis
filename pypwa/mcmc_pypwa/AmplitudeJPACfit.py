...
#        Vs = self.__param_to_Vs(params, wave1, wave1)
        real_string = f"r.{self.__elm['e'][wave1]}.{self.__elm['l'][wave1]}.{self.__elm['m'][wave1]}"
        imag_string = f"i.{self.__elm['e'][wave1]}.{self.__elm['l'][wave1]}.{self.__elm['m'][wave1]}"
        Vs = npy.complex(params[real_string], params[imag_string])
    ...
    def __param_to_Vs(self, params, wave1):
        ...
        real_string = f"r.{self.__elm['e'][wave1]}.{self.__elm['l'][wave1]}.{self.__elm['m'][wave1]}"
        imag_string = f"i.{self.__elm['e'][wave1]}.{self.__elm['l'][wave1]}.{self.__elm['m'][wave1]}"
        value = npy.complex(params[real_string], params[imag_string])
    ...
    def Phasediff(self, params, wave1, wave2):
        ...
        for index, i in enumerate([wave1, wave2]):
            real_string = f"r.{self.__elm['e'][i]}.{self.__elm['l'][i]}.{self.__elm['m'][i]}"
            imag_string = f"i.{self.__elm['e'][i]}.{self.__elm['l'][i]}.{self.__elm['m'][i]}"
            value[index] = npy.complex(params[real_string], params[imag_string])
